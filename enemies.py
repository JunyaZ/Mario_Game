...
class Enemy(pg.sprite.Sprite):
    """This is a Base class for all enemies (Goombas, Koopas, etc.)"""
    def __init__(self):
        pg.sprite.Sprite.__init__(self)

    def setup_enemy(self, x, y, direction, name, setup_frames):
        """Sets up different values for enemy"""
        self.sprite_sheet = initialize.GFX['smb_enemies_sheet']
        self.frames = []
        self.frame_index = 0
        ...
        self.rect = self.get_image(self,width,height)
        self.rect.x = x
        self.rect.bottom = y
        self.set_velocity()
    ...
    def get_image(self, x, y, width, height):
        ...
        image.set_colorkey(Dir.BLACK)


        image = pg.transform.scale(image,
                                   (int(rect.width*Dir.SIZE_MULTIPLIER),
                                    int(rect.height*Dir.SIZE_MULTIPLIER)))
        return image


    def handle_state(self):
        """Enemy behavior based on state"""
        if self.state == Dir.WALK:
            self.walking()
        elif self.state ==Dir.FALL:
            self.falling()
        elif self.state == Dir.JUMPED_ON:
            self.jumped_on()
        elif self.state == Dir.SHELL_SLIDE:
            self.shell_sliding()
        elif self.state == Dir.DEATH_JUMP:
            self.death_jumping()
    ...
    def update(self, game_info, *args):
        ...
        self.current_time = game_info[Dir.CURRENT_TIME]
        self.handle_state()
        self.animation()
...
class Goomba(Enemy):
    ...
    def __init__(self, y=Dir.GROUND_HEIGHT, x=0, direction=Dir.LEFT, name='goomba'):
        Enemy.__init__(self)
...
class Koopa(Enemy):
    ...
    def __init__(self, y=Dir.GROUND_HEIGHT, x=0, direction=Dir.LEFT, name='koopa'):
        Enemy.__init__(self)
        self.setup_enemy(x, y, direction, name, self.setup_frames)
